...
The layer's only external dependencies are data/protocol crates — `nostr_sdk`
and, in `nip38`, the `nowhear::Track` value type — never a UI, I/O, or framework
crate. `nowhear::Track` is consumed as a plain value to build a `MusicStatus`;
the media source that produces it lives in `infrastructure`.
...
- `model::editor` — the compose/reply buffer. It wraps `tui_textarea::TextArea`,
  which fuses edit state with rendering and owns the undo/redo history, so this
  is the **one `model` component coupled to UI/input crates** (`tui_textarea`,
  `ratatui::widgets`, and `crossterm` key events). Splitting state from view was
...
`runtime` is the sole entry point for outside input, but the translation is
sometimes a thin wrapper rather than a full conversion: `KeyInput` /
`ProcessTextAreaInput` carry a raw `crossterm::KeyEvent` (text editing needs the
unmapped key for the `editor`'s `TextArea`), and `Media` carries a raw `nowhear`
result. For these payloads `application` is knowingly coupled to `crossterm` /
`nowhear`.
...
4. **One message vocabulary.** `AppMsg` is defined in `application`; only the
   `runtime` translates the outside world into it (sometimes as a thin wrapper
   around a raw `crossterm` / `nowhear` value — see Messages).
5. **Views are read-only.** `presentation` renders `AppState` and nothing more.
6. **`runtime` is the only shell.** Framework *driver* concerns — composition,
   subscriptions, the `tears::Application` impl — live only here. One framework
   type crosses inward by design: `application` use cases return
   `Command<AppMsg>` (the `tears` effect type), the sanctioned seam between use
   case and shell. Otherwise inner layers stay framework-light and testable,
   with `model::editor` (UI-coupled via `tui_textarea`) the one documented
   exception.
