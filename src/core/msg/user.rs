...
mod tests {
    ...
    fn create_test_profile() -> Profile {
        Profile {
            ...
            metadata: nostr_sdk::Metadata::new()
                ...
                .about("Test bio")
                .picture("https://example.com/avatar.jpg".parse().unwrap()),
            created_at: nostr_sdk::Timestamp::now(),
        }
    }

    #[test]
    fn test_user_msg_frequent_detection() {
    ...
    #[test]
    fn test_user_msg_serialization() {
        let pubkey = Keys::generate().public_key();
        let profile = create_test_profile();
        let msg = UserMsg::UpdateProfile(pubkey, profile);

        let serialized = serde_json::to_string(&msg).unwrap();
        let deserialized: UserMsg = serde_json::from_str(&serialized).unwrap();
